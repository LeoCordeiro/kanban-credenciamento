...
export function SlaConfigModal({ slas, onClose }: { slas: SlaColuna[]; onClose: () => void }) {
  ...
  const [valores, setValores] = useState<Record<string, string>>(() =>
    Object.fromEntries(COLUNAS.map(c => [c.id, String(globais.find(s => s.coluna === c.id)?.max_horas ?? '')]))
  )
  ...
  async function salvar() {
    ...
    for (const c of COLUNAS) {
      const horas = parseInt(valores[c.id], 10)
      const existente = globais.find(s => s.coluna === c.id)
      if (horas > 0) {
        if (existente) {
          if (existente.max_horas !== horas) await supabase.from('sla_colunas').update({ max_horas: horas }).eq('id', existente.id)
        } else {
          await supabase.from('sla_colunas').insert({ plataforma_id: null, coluna: c.id, max_horas: horas })
        }
      } else if (existente) {
    ...
    <Modal titulo="SLA por coluna" onClose={onClose} maxWidth="max-w-sm">
      <div className="p-5 space-y-3">
        <p className="text-xs text-text-secondary">
          Horas máximas que um card pode ficar parado em cada coluna. Estourou, o card ganha destaque no board e entra em Atrasos. Vazio = sem SLA.
        </p>
        {COLUNAS.map(c => {
          const h = parseInt(valores[c.id], 10)
          return (
            <div key={c.id} className="flex items-center gap-3">
              ...
              <input
                ...
                placeholder="—"
                className="w-20 px-2 py-1 bg-surface-sunken border border-border rounded-lg text-sm font-mono text-text-primary text-center placeholder:text-text-muted focus:outline-none focus:border-btn-primary"
              />
              <span className="text-xs text-text-muted w-14">
                {h > 48 ? `horas (${formatHoras(h)})` : 'horas'}
              </span>
            </div>
          )
        })}
